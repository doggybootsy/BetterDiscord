...
import {BadgeCheckIcon, CircleHelpIcon, EyeIcon, GithubIcon, GlobeIcon, Trash2Icon} from "lucide-react";
import type {StoreAddon} from "@modules/addonstore";
import {TagContext} from "./shared";

const {useCallback, useMemo, useState, useEffect, useContext} = React;

function formatNumberWithSuffix(value: number) {
    if (value === 0) return "0";
...
export default function AddonCard({addon, isEmbed}: {addon: StoreAddon, isEmbed?: boolean;}) {
    ...
    const {downloads, likes} = useMemo(() => ({
        downloads: t("Addons.downloadCount", {count: formatNumberWithSuffix(downloadCount) as any}),
        likes: t("Addons.likeCount", {count: formatNumberWithSuffix(addon.likes) as any}),
    }), [addon, downloadCount]);
        ...
        >
            ...
            <div className="bd-addon-store-card-body">
                <div className="bd-addon-store-card-name">
                    <DiscordModules.Tooltip text={t("Addons.official")}>
                        {(props) => (
